...
DROP FUNCTION IF EXISTS dctoAntiguedad;

DELIMITER //

CREATE FUNCTION dctoAntiguedad(fecha_operacion DATE,fecha_registro DATE)
RETURNS DECIMAL(5, 4)
DETERMINISTIC
BEGIN
DECLARE dias_antiguedad INT;
DECLARE periodos_adicionales INT;
DECLARE descuento DECIMAL(5,4);
SET dias_antiguedad = DATEDIFF(fecha_operacion, fecha_registro);
IF dias_antiguedad < 180 THEN
SET descuento = 0.0;
ELSE 
SET periodos_adicionales = FLOOR((dias_antiguedad - 180) / 180);
SET descuento = (5 + periodos_adicionales) / 100.0;
END IF;
RETURN descuento;
END //
...
BEGIN
    ...
    SELECT  fRegistro INTO @fechaRegistro FROM clientes WHERE id_cliente=NEW.fk_cliente;
	SET v_descuento = dctoAntiguedad(fechaRegistro, a.fecha_inicio);
    
    SELECT c.precio_dia,
           # aquí tendría que hacer select de registro id3 y fecha inicio IFNULL(a.descuento, 0),
           v_descuento,
    ...
    SET v_final = v_base - (v_base*v_descuento);
...
-- CLIENTES
INSERT INTO Clientes (nombre, apellidos, telefono, dni, email, fRegistro)
VALUES 
('Ana','Pérez Gómez','600111222','11111111A','ana@example.com','2023-09-12'),
('Luis','García López','600222333','22222222B','luis@example.com','2024-09-12'),
('Marta','Santos Ruiz','600333444','33333333C','marta@example.com','2021-09-12'),
('Carlos','Nieves Díaz','600444555','44444444D','carlos@example.com','2020-09-12'),
('Lucía','Martín Vega','600555666','55555555E','lucia@example.com','2022-09-12');
...
INSERT INTO Alquiler VALUES (null,3,4,"2025-09-02 18:05:15","2025-09-12 18:05:15",0,0,"2025-10-02 18:05:15","2025-10-02 18:05:15");
